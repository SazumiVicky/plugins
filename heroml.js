...
let handler = async(m, { conn, text }) => {
  ...
  await m.reply('Searching...')
    let res = await fetch(`http://api.lannn.me/api/search/heroml?hero=${text}&apikey=P5hoOJWE`)
    let json = await res.json()
    if (res.status !== 200) throw await res.text()
    if (!json.status) throw json
    let thumb = await (await fetch('https://i.pinimg.com/564x/05/98/bf/0598bfe852f9fb27fed4913fb90e5ca8.jpg')).buffer()
    let hasil = `*── 「 HERO ML 」 ──*
